document.addEventListener('DOMContentLoaded',function(){
  ...
  function render(){sortData();const rows=paginate(filtered());tbody.innerHTML='';rows.forEach(r=>{const tr=document.createElement('tr');tr.innerHTML=`<td>${r.no}</td><td>${r.nik}</td><td>${r.nama}</td><td><span class="badgeDept">${r.dept}</span></td>`;tbody.appendChild(tr)})}
  ...
  function ensureBgm(){ if(!bgm){ bgm=document.getElementById('bgm'); if(!bgm){ bgm=document.createElement('audio'); bgm.id='bgm'; bgm.src='assets/queen_champions.m4a'; bgm.preload='auto'; document.body.appendChild(bgm) } } return bgm }
  function startMusic(){ if(musicOn) return; const a=ensureBgm(); a.loop=true; a.volume=0.5; const p=a.play(); musicOn=true; if(p&&typeof p.catch==='function'){ p.catch(()=>{ const kick=()=>{ a.play(); document.removeEventListener('pointerdown',kick); document.removeEventListener('keydown',kick); document.removeEventListener('touchstart',kick) }; document.addEventListener('pointerdown',kick); document.addEventListener('keydown',kick); document.addEventListener('touchstart',kick,{passive:true}) }) } }
